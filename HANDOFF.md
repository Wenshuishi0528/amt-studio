...
## 当前唯一下一步：试听 Task 009B2E

在软件中打开 `1-07-still-love-her-失われた風景`，选择结果
`task009b2e-muscriptor-gap-v2-review`。两个轨道必须保持分开：

- `voice raw（原始，不修改）`：原始 254 音符主唱候选；
- `voice gap candidate（仅补漏候选）`：本次 184 音符定向补漏结果。

依次试听 `0:00–0:33.35`、`1:03.55–1:30.69`、
`1:44.52–2:11.41`、`3:15.14–4:32.50` 和
`4:32.50–5:49.15`。只需要判断每段“补回的旋律是否正确、增加了多少明显错音、
该段是否本来就没有主唱”。当前客观结果是：片头负对照 0 个候选；两个中段各
52 个；尾段前半 80 个；尾段后半 0 个。非负对照区域的候选时间覆盖为
60.23/208.043719 秒（28.95%），不能解释为准确率。
...
在上述试听完成前，不启动 BS-Roformer、GAME、训练、融合或自动合并。
...
- 对 MuScriptor 多轨默认打开 `voice` 主唱候选，同时保留所有伴奏轨；
...
- 分别导出当前编辑音轨、当前可听混音和完整修正版多轨 MIDI；
...
`make check` 通过 247 项 Python 和 24 项 Swift 测试；两个显式启用的真实项目
集成测试也通过。聚焦 P0/P1 的代码检查未发现剩余阻塞问题，最新版已用本机
