...
Task 009B3Q 将已经完成但尚未发布的 Task 009B3O/P 整理为 AMT Studio
`0.2.1 (3)`：包括 Slurm 完成任务自动转查 `sacct` 的恢复修复，以及可记忆的
“最近完成”折叠功能。单独进行中的 Task 007D 数据准入研究不属于本次发布，私人
歌曲、结果、数据集、权重、Hyak 身份和日志也不会进入公开提交或附件。发布仍是
未公证、需配合仓库后端使用的开发者/测试者 Private Beta。`make check` 通过
290 个 Python 和 60 个 Swift 测试（3 个私有实时测试按预期跳过），干净的 Xcode
Release、独立打包、arm64/版本/严格签名检查和启动冒烟测试均通过。

Task 009B3P 给音乐库的“最近完成”加入了可记忆的收起/展开功能。点击分组标题即可
折叠大量已完成歌曲，右侧箭头显示当前状态；退出并重新打开软件后仍保持上次选择。
搜索音乐时会临时显示匹配的已完成歌曲，清空搜索后恢复原先的折叠状态，避免出现
“明明搜到了但列表看不见”的情况。这个改动只影响列表显示，不修改项目、任务队列
或 Hyak 状态。

Task 009B3O 修复了任务完成后偶发的
`slurm_load_jobs error: Invalid job id specified` 弹窗。Slurm 会把已完成任务
从活动 `squeue` 视图移除，这不代表 SSH 失败；软件现在只在遇到这一条精确错误时
自动改查 `sacct` 历史账本，其他网络、登录或调度错误仍正常报告。真实受影响项目
已确认是 `COMPLETED / 0:0`，结果成功取回且没有重新提交作业。回归测试覆盖相同
错误，`make check` 通过 297 个 Python 和 60 个 Swift 测试（3 个私有实时测试
按预期跳过）。

Task 009B3N 把 GitHub 首页从已经过时的研究框架说明改成中英双语产品简介，准确说明
“歌曲 → 可编辑多轨 MIDI + 可选主旋律”的三步流程、现有编辑/补漏/版本管理/导出
功能、Hyak 与本机计算边界、安装方式、模型误差、隐私和许可证。旧 README 已原样
备份到 `docs/archive/README_PRE_V0.2.0.md`，v0.2.0 的中英双语发布说明位于
`docs/releases/V0.2.0.md`。公开范围不包含私人 Hyak 身份与路径、凭据、歌曲、
转录结果、数据集、权重或集群日志；Mac 附件仍是未公证、需配合仓库使用的
开发者/测试者 Private Beta。

Task 009B3M 在左侧“计算”区域加入“查看 Hyak 资源状态”。窗口会分别显示当前账号
正在运行、排队及其他状态的任务数，并按兼容 GPU 列出空闲、部分占用、已占用和
不可用节点快照；同时根据设置里的任务时限运行 Slurm `sbatch --test-only`，
显示预计启动时间并标出当前推荐 GPU。这里不提交占位作业、不占 GPU，也不会修改
现有任务；界面明确区分瞬时 `sinfo` 快照与不保留资源的调度估算，并且不显示私人
Hyak 用户名、主机名或账号名。`make check` 通过 296 个 Python 和 60 个 Swift
测试（3 个私有实时测试按预期跳过），Xcode 与发布版 App 均构建成功。

Task 009B3L 修复了进行中任务计时周期性归零的问题。旧界面错误使用会随每次
Slurm 刷新变化的状态文件修改时间；现在统一使用任务首次提交时写入且保持不变的
`submitted_at`，列表每秒更新但刷新状态不会重置。处理流程页同时新增“任务已用时”
和“预计完成”时间范围；估算综合歌曲长度、任务类型、GPU、排队估计与当前阶段，并
明确提示它会受自动补漏和集群拥堵影响。`make check` 通过 295 个 Python 和 59 个
Swift 测试（3 个私有实时测试按预期跳过），Xcode 与发布版 App 构建成功。本任务
没有提交、取消或修改任何 Hyak 作业。

Task 009B3K 已把之前选定的浅蓝唱片、波形与钢琴卷帘封面正式设为 macOS App
图标。项目现在包含标准多分辨率 `.icns`；Xcode 工程与独立发布打包脚本都会把它
放入 App，并通过 `CFBundleIconFile` 注册。`make check`、正式 Xcode 构建、发布版
打包和严格签名校验均通过。重新打开打包后的 App 后，Finder、Dock 与软件弹窗应显示
同一张封面，而不是空白通用图标。

Task 009B3J 已把 Hyak 时限改成按歌曲长度自动设置：7 分钟以内至少 1 小时，
超过 7 分钟至少 2 小时，超过 14 分钟至少 3 小时；超过 21 分钟不再静默提交，
而是弹出窗口显示歌曲时长，并让用户确认 1–24 小时。设置里的时长仍有效，但现在
表示“最低时限”，不会压低自动计算结果。队列会显示每首歌冻结后的实际时限。

TIMEOUT 项目现在可从音乐库菜单选择“从检查点继续”。当前安全范围是 MuScriptor
完整多轨任务：只有远端已存在完整原始多轨 bundle 时才续算，旧超时补漏目录会以
原 Job ID 归档，新的 Slurm 作业只重启自动补漏阶段，不重跑昂贵的整曲识别。
如果最终结果其实已经生成，软件直接取回；如果检查点不存在或任务类型不支持，
软件会明确拒绝并提示完整重试。`make check` 通过 295 个 Python 和 57 个 Swift
测试（3 个私有实时测试按预期跳过），正式 Xcode 工程与发布版 App 也已成功构建。
没有为本任务上传歌曲或新建模型作业，原有 Hyak 恢复任务未被修改。

Task 009B3I 已把音轨管理扩展到不同歌曲之间：先打开来源歌曲、识别版本和音轨，再在
“管理版本与音轨”里选择目标歌曲与目标识别版本。软件只复制，不移动或删除来源；
目标歌曲会新增一个已验证的 `custom-*` 版本。所有音符的开始和结束秒数原样保留，
即使超过目标歌曲结尾也不会裁剪；单轨和整个版本 MIDI 导出同样保留这些音符。目标
歌曲的原音频试听仍按它自己的真实时长结束，不会伪造或拉长音频。来源项目、版本、
音轨、事件和模型/编辑链均写入溯源。

同一任务也移除了 Mac 客户端原先的单活动 Hyak 作业门槛。这个门槛是软件限制，
不是已经证实的 Hyak“最多两个”配额。现在上传与 `sbatch` 仍逐首串行，以避免代码
同步和文件传输互相冲突；每首提交确认后会立即提交下一首。实际同时运行多少由
Slurm/QOS 决定，没有资源的作业保持 `PENDING`。所有活动项目由一个持久化后台
监控器逐个查询和取回结果；本机 CPU/GPU 仍串行。`make check` 通过 294 个 Python
和 56 个 Swift 测试（3 个私有实时测试按预期跳过）。实现过程没有提交歌曲、模型
或 Hyak 作业。唯一一次 `/review` 因开始越界检查已暂停的 Task 007D 而被终止；
随后只做相关状态检查并修复两个 P1：后台任务终止后会继续释放本地等待任务，删除
当前查看项目也不会停止其他活动 Hyak 项目的监控；没有运行第二次或扩大审查。

Task 009B3H 已采用所有者选择的最后一版天空蓝金封面，并将其作为项目资源显示在
首页、侧栏品牌区和设置“关于”区域。软件版本从 `0.1.0 (1)` 更新为 Private Beta
`0.2.0 (2)`，作者统一显示为 `wenshuishi26`；plist、Xcode 和 Python 包元数据
保持一致。签名后的 `.app` 已实际包含同一张 1254 x 1254 PNG，源码与成品资源的
SHA-256 均为
`ea9d44fd188d9a9ab915633ba06ec3171ea338fc7b69791670d20f2bddf26c23`。
`make check` 通过 294 个 Python 和 52 个 Swift 测试（3 个私有实时测试按预期
跳过），格式、plist、打包、资源哈希和签名验证通过。本任务没有提交或修改任何
Hyak 作业。

Task 009B3G 已加入真正的顺序多歌曲队列。现在“添加歌曲”可以一次多选，每首歌会
在入队时冻结自己的识别模式、计算位置和 Hyak 时限；软件仍然只允许一个活动任务，
当前任务进入任一终态后才自动提交下一首。等待项及文件授权会在 Mac 本地持久化，
提交确认中途退出的项目不会自动重发，而是要求手动重试以避免重复 Slurm 作业。
侧栏可查看顺序、状态、失败原因，并对失败项重试或移除。任务失败不会阻塞后面的
等待歌曲，登录过期则暂停队列等待用户在 Terminal 完成登录。新版已打包并重启；
本机原先没有保存的待提交队列，因此没有额外提交模型任务，也没有替换已有远端
任务。`make check` 通过 294 个 Python 和 52 个 Swift 测试（3 个私有实时测试按
预期跳过）。
